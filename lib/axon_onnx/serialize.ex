defmodule AxonOnnx.Serialize do
  ...
  # parameterized equivalenet operation in Axon (e.g. add, multiply, etc.), we need
  ...
  defp get_inputs(%Graph{input: inputs}, params, dimensions) do
    Enum.reduce(inputs, %{}, fn %ValueInfo{name: name, type: %Type{value: value}}, acc ->
  ...
  defp get_params(%Graph{initializer: initializer}) do
    Enum.reduce(initializer, %{}, fn %T{name: name} = tensor, params ->
      Map.put(params, name, tensor!(tensor))
    end)
  end

  defp get_outputs(%Graph{output: outputs}) do
    Enum.map(outputs, fn %ValueInfo{name: name} -> name end)
  end
  ...
  defp get_nodes(pruned_nodes, inp, params, used_params) do
    Enum.reduce(pruned_nodes, {inp, used_params}, fn %Node{op_type: op_type} = op_node,
      ...
      case op_type do
        ...
        "AveragePool" ->
          # TODO(seanmor5): unify window pooling
          raise "unsupported op AveragePool"

        "BatchNormalization" ->
          to_axon_batch_norm(op_node, axon, params, used_params)

        "BitShift" ->
          raise "unsupported op BitShift"

        "Cast" ->
          raise "unsupported op Cast"
        ...
        "Clip" ->
          to_axon_clip(op_node, axon, params, used_params)

        "Compress" ->
          raise "unsupported op Compress"

        "Concat" ->
          raise "unsupported op Concat"

        "ConcatFromSequence" ->
          raise "unsupported op ConcatFromSequence"

        "Constant" ->
          to_axon_constant(op_node, axon, params, used_params)

        "ConstantOfShape" ->
          # TODO(seanmor5): unify with to_axon_constant
          raise "unsupported op ConstantOfShape"

        "Conv" ->
          to_axon_conv(op_node, axon, params, used_params)

        "ConvInteger" ->
          # TODO(seanmor5): unify conv
          raise "unsupported op ConvInteger"

        "ConvTranspose" ->
          # TODO(seanmor5): conv transpose
          raise "unsupported op ConvTranspose"
        ...
        "CumSum" ->
          raise "unsupported op CumSum"

        "DepthToSpace" ->
          raise "unsupported op DepthToSpace"

        "DequantizeLinear" ->
          raise "unsupported op DequantizeLinear"

        "Det" ->
          raise "unsupported op Det"

        "Div" ->
          to_axon_binary_op(op_node, axon, params, used_params, fn {x, y} -> Nx.divide(x, y) end)

        "Dropout" ->
          raise "unsupported op Dropout"

        "Einsum" ->
          raise "unsupported op Einsum"
        ...
        "Expand" ->
          raise "unsupported op Expand"

        "EyeLike" ->
          raise "unsuporrted op EyeLike"
        ...
        "GRU" ->
          raise "unsupported op GRU"

        "Gather" ->
          raise "unsupported op Gather"

        "GatherElements" ->
          raise "unsupported op GatherElements"

        "GatherND" ->
          raise "unsupported op GatherND"
    ...
    {axon_kernel, units} =
      cond do
        ...
        Map.has_key?(axon, kernel) ->
          %{output_shape: shape} = kernel = axon[kernel]
          {kernel, elem(shape, 1)}
  ...
  defp to_axon_global_pool(
    ...
    updated_axon =
      case op_type do
        "GlobalAveragePool" ->
          Map.put(axon, output_name, Axon.global_average_pool(inp, name: output_name))
  ...
  defp to_axon_constant(
      ...
      fn _ ->
        cond do
          ...
          constant_options["value"] ->
            tensor!(constant_options["value"])

          constant_options["value_float"] ->
            Nx.tensor(constant_options["value_float"], type: {:f, 32}, backend: Nx.Defn.Expr)

          constant_options["value_floats"] ->
            Nx.tensor(constant_options["value_floats"], type: {:f, 32}, backend: Nx.Defn.Expr)

          constant_options["value_int"] ->
            Nx.tensor(constant_options["value_int"], type: {:s, 64}, backend: Nx.Defn.Expr)

          constant_options["value_ints"] ->
            Nx.tensor(constant_options["value_ints"], type: {:s, 64}, backend: Nx.Defn.Expr)
    ...
    # TODO(seanmor5): Use Axon.layer when shape inference is supported
    # TODO(seanmor5): Should layer support blank inputs for constants?
    updated_axon = Map.put(axon, output_name, Axon.nx(Axon.input({nil, 1}), fun, name: output_name))
  ...
  defp to_axon_transpose(
    ...
    updated_axon =
      Map.put(axon, output_name, Axon.transpose(inp, permutation: permutation, name: output_name))
  ...
  defp to_axon_clip(%Node{op_type: "Clip", input: [input], attribute: attrs, output: [output_name]}, axon, params, used_params) do
    clip_options = options!(attrs)

    inp = input_or_param!(input, params, axon, used_params)

    min = clip_options["min"]
    max = clip_options["max"]

    fun = fn input ->
      Nx.clip(input, min, max)
    end
  ...
  defp tensor!(%T{data_type: dtype, dims: dims} = tensor) do
