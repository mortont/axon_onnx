defmodule SerializeTest do
  ...
  describe "serializes dense layers" do
    test "dense with defaults" do
      model = Axon.input({1, 32}, "input") |> Axon.dense(10)
      serialize_and_test_model!(model, {1, 32}, num_tests: 3, name: "default_dense")
    end

    test "dense with name" do
      model = Axon.input({1, 32}, "input") |> Axon.dense(10, name: "dense")
      serialize_and_test_model!(model, {1, 32}, num_tests: 3, name: "dense_with_name")
    end

    # TODO: Re-add when we have layer metadata
    # test "dense with use_bias false" do
    #   model = Axon.input({1, 32}, "input") |> Axon.dense(10, name: "dense", use_bias: false)
    #   serialize_and_test_model!(model, num_tests: 3, name: "dense_no_bias")
    # end

    test "dense with activation" do
      model = Axon.input({1, 32}, "input") |> Axon.dense(10, activation: :relu)
      serialize_and_test_model!(model, {1, 32}, num_tests: 3, name: "dense_with_activation")
    end

    test "multiple dense layers" do
      model =
        Axon.input({1, 32}, "input")
        |> Axon.dense(10, name: "dense_1")
        |> Axon.dense(1, name: "dense_2")

      serialize_and_test_model!(model, {1, 32}, num_tests: 3, name: "multi_dense")
    end
  end
  ...
  describe "serializes activations" do
    test "onnx supported activations" do
      ...
      for {op, onnx_op} <- supported_activations do
        model = Axon.input({1, 10}, "input") |> Axon.activation(op)
        serialize_and_test_model!(model, {1, 10}, num_tests: 3, name: onnx_op)
      end
    end
  end
  ...
  describe "serializes pooling layers" do
    ...
    test "max_pool with same padding" do
      model =
        Axon.input({1, 3, 7, 7}, "input") |> Axon.max_pool(kernel_size: {2, 1}, padding: :same)
    ...
    test "avg_pool with same padding" do
      model =
        Axon.input({1, 3, 7, 7}, "input") |> Axon.avg_pool(kernel_size: {2, 1}, padding: :same)
    ...
    test "lp_pool with same padding" do
      model =
        Axon.input({1, 3, 7, 7}, "input") |> Axon.lp_pool(kernel_size: {2, 1}, padding: :same)
    ...
    test "lp_pool with norm" do
      model = Axon.input({1, 3, 7, 7}, "input") |> Axon.lp_pool(kernel_size: {2, 1}, norm: 3)
  ...
  describe "serializes global pooling" do
    ...
    test "global_max_pool with conv and dense" do
      ...
      serialize_and_test_model!(model, {1, 3, 7, 7},
        num_tests: 3,
        name: "global_max_pool_conv_dense"
      )
    end

    test "global_max_pool keep_axes false" do
      model = Axon.input({1, 3, 7, 7, 7}, "input") |> Axon.global_max_pool(keep_axes: false)

      serialize_and_test_model!(model, {1, 3, 7, 7, 7},
        num_tests: 3,
        name: "global_max_pool_no_axes"
      )
    end

    test "global_avg_pool1d with defaults" do
      model = Axon.input({1, 3, 7}, "input") |> Axon.global_avg_pool()
      serialize_and_test_model!(model, {1, 3, 7}, num_tests: 3, name: "global_avg_pool1d_default")
    end

    test "global_avg_pool2d with defaults" do
      model = Axon.input({1, 3, 7, 7}, "input") |> Axon.global_avg_pool()

      serialize_and_test_model!(model, {1, 3, 7, 7},
        num_tests: 3,
        name: "global_avg_pool2d_default"
      )
    end

    test "global_avg_pool3d with defaults" do
      model = Axon.input({1, 3, 7, 7, 7}, "input") |> Axon.global_avg_pool()

      serialize_and_test_model!(model, {1, 3, 7, 7, 7},
        num_tests: 3,
        name: "global_avg_pool3d_default"
      )
    end
    ...
    test "global_avg_pool with conv and dense" do
      ...
      serialize_and_test_model!(model, {1, 3, 7, 7},
        num_tests: 3,
        name: "global_avg_pool_conv_dense"
      )
    end

    test "global_avg_pool keep_axes false" do
      model = Axon.input({1, 3, 7, 7, 7}, "input") |> Axon.global_avg_pool(keep_axes: false)

      serialize_and_test_model!(model, {1, 3, 7, 7, 7},
        num_tests: 3,
        name: "global_avg_pool_no_axes"
      )
    end

    test "global_lp_pool1d with defaults" do
      model = Axon.input({1, 3, 7}, "input") |> Axon.global_lp_pool()
      serialize_and_test_model!(model, {1, 3, 7}, num_tests: 3, name: "global_lp_pool1d_default")
    end

    test "global_lp_pool2d with defaults" do
      model = Axon.input({1, 3, 7, 7}, "input") |> Axon.global_lp_pool()

      serialize_and_test_model!(model, {1, 3, 7, 7},
        num_tests: 3,
        name: "global_lp_pool2d_default"
      )
    end

    test "global_lp_pool3d with defaults" do
      model = Axon.input({1, 3, 7, 7, 7}, "input") |> Axon.global_lp_pool()

      serialize_and_test_model!(model, {1, 3, 7, 7, 7},
        num_tests: 3,
        name: "global_lp_pool3d_default"
      )
    end

    test "global_lp_pool with norm" do
      model = Axon.input({1, 3, 7, 7}, "input") |> Axon.global_lp_pool(norm: 3)

      serialize_and_test_model!(model, {1, 3, 7, 7},
        num_tests: 3,
        name: "global_lp_pool_with_norm"
      )
    end
    ...
    test "global_lp_pool with conv and dense" do
      ...
      serialize_and_test_model!(model, {1, 3, 7, 7},
        num_tests: 3,
        name: "global_lp_pool_conv_dense"
      )
    end

    test "global_lp_pool keep_axes false" do
      model = Axon.input({1, 3, 7, 7, 7}, "input") |> Axon.global_lp_pool(keep_axes: false)

      serialize_and_test_model!(model, {1, 3, 7, 7, 7},
        num_tests: 3,
        name: "global_lp_pool_no_axes"
      )
    end
  end
end
